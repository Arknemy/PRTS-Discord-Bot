	...
	if str(ctx.author.id) in id_list:
		if arg.startswith('question '):
			...
			if split_question[0] in questions_list:
				await ctx.send('This question is already added.', delete_aftxer = 15)
...
async def highlow(ctx):
	...
	index = random.randrange(0, deck_count)
	last_card = deck[index]
	del deck[index]
	deck_count -= 1
	embedVar = discord.Embed(description = 'Card remaining: {}\nScore: {}'.format(deck_count, score), color = 0xe3e3e3)
	image_file = discord.File('poker_cards/{}.png'.format(deck[index]), filename = 'image.png')
	embedVar.set_image(url = "attachment://image.png")
	await ctx.send(file = image_file, embed = embedVar)
	...
	while deck:
		...
		await bot.wait_for('message', check = lambda message: message.author == ctx.author, message = 'high')
		...
		# new code
		new_card = deck[index]
		#if last_card[0] or new_card[0] not 'J' and last_card[0] or new_card[0] not 'Q' and last_card[0] or new_card[0] not 'K':
		if new_card[0] == 'J' or new_card[0] == 'Q' or new_card == 'K':
			if last_card[0] != 'J' and last_card[0] != 'Q' and last_card[0] != 'K':
				score += 1
...
async def on_message(ctx):
	...
	if ctx.author.id == 221724337902845954:
		...
		if ctx.content == 'who\'s a cutie':
			...
			await ctx.channel.send(file=discord.File('images/{}.png'.format('horny')))
